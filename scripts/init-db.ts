...
async function main() {
  ...
  const receipt=buildReceipt(events,{repositoryId:"repo_demo",prNumber:412,title:"Prevent cart race conditions",headSha:"4b46ac",outcome:"merged",outcomeAt:new Date(now-86400000).toISOString()}); await pool.query("INSERT INTO pull_requests(repository_id,number,branch,head_sha,title,state,outcome,merged_at,closed_at,updated_at) VALUES('repo_demo',412,'fix/cart-race','4b46ac','Prevent cart race conditions','closed','merged',NOW()-INTERVAL '1 day',NOW()-INTERVAL '1 day',NOW()) ON CONFLICT(repository_id,number) DO NOTHING"); await pool.query("INSERT INTO receipts(repository_id,pr_number,receipt) VALUES('repo_demo',412,$1::jsonb) ON CONFLICT(repository_id,pr_number) DO NOTHING",[JSON.stringify(receipt)]);
