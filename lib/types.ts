...
export type Receipt = { repositoryId: string; prNumber: number; title: string; headSha: string; totalCost: number; confidence: number; eventCount: number; models: ModelBreakdown[]; explanation?: string; observation?: ReceiptObservation; updatedAt: string };
...
export type RepositoryOverview = Dashboard & { modelSpend: ModelBreakdown[]; spendTrend: SpendPoint[]; recentEvents: UsageEvent[]; lastActivityAt?: string; telemetryHealthy: boolean };
