...
export function buildReceipt(events: UsageEvent[], input: { repositoryId: string; prNumber: number; title: string; headSha: string; outcome?: Receipt["outcome"]; outcomeAt?: string }): Receipt {
  ...
  return { ...input, totalCost: round(totalCost), confidence: exactContext, eventCount: events.length, models: [...byModel.values()].map((model) => ({ ...model, costUsd: round(model.costUsd) })).sort((a, b) => b.costUsd - a.costUsd), actors: [...byActor.values()].map((actor) => ({ ...actor, costUsd: round(actor.costUsd) })).sort((a, b) => b.costUsd - a.costUsd), updatedAt: new Date().toISOString() };
}
...
export function receiptMarkdown(receipt: Receipt, dashboardUrl: string): string {
  ...
  const observation = receipt.observation?.category === "attribution_quality" ? "" : receipt.observation ? `\n> **Governor observation - ${receipt.observation.title}.** ${receipt.observation.explanation}${receipt.observation.impactUsd ? ` Estimated impact: ${money(receipt.observation.impactUsd)}.` : ""}\n` : "";
  const workContext = receipt.workContext ? workContextMarkdown(receipt) : "";
  return `<!-- governor-cost-receipt -->\n## Governor cost receipt\n\n**Estimated Codex cost: ${money(receipt.totalCost)}** - ${receipt.eventCount} usage events${actors ? `\n\n${actors}` : ""}${outcome ? `\n\n${outcome}` : ""}${workContext}\n${modelLines}\n${observation}\n${receipt.explanation ? `> ${receipt.explanation}\n\n` : ""}These are transparent token-rate calculations from recorded usage and effective rates. Governor never stores prompts, responses, generated code, raw PR comments, or repository file contents. [View calculation](${dashboardUrl})`;
}
