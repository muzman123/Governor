...
export async function repositoryOverview(store: GovernorStore, repositoryId: string): Promise<RepositoryOverview> {
  const [dashboard,events,agentTokenConfigured]=await Promise.all([store.getDashboard(repositoryId),store.getEvents(repositoryId),store.hasActiveAgentToken(repositoryId)]); const byModel=new Map<string,ModelBreakdown>(); const byActor=new Map<ActorBreakdown["actorType"],ActorBreakdown>(); const byDay=new Map<string,number>();
  ...
  return {...dashboard,modelSpend:[...byModel.values()].map((model)=>({...model,costUsd:Math.round(model.costUsd*1_000_000)/1_000_000})).sort((a,b)=>b.costUsd-a.costUsd),actorSpend:[...byActor.values()].map((actor)=>({...actor,costUsd:Math.round(actor.costUsd*1_000_000)/1_000_000})).sort((a,b)=>b.costUsd-a.costUsd),spendTrend,recentEvents,lastActivityAt:recentEvents[0]?.occurredAt,telemetryHealthy:Boolean(recentEvents.length),agentTokenConfigured};
}
