...
export class PostgresGovernorStore implements GovernorStore {
  ...
  async getRates() {
    const r=await this.pool.query("SELECT model,effective_from::text AS \"effectiveFrom\",input_per_mtok::float AS \"inputPerMTok\",output_per_mtok::float AS \"outputPerMTok\",cached_input_per_mtok::float AS \"cachedInputPerMTok\" FROM model_rates");
    // Database values override bundled defaults. Keeping defaults here prevents a
    // newly supported Codex model from being silently rejected before db:init runs.
    const rates=new Map(DEFAULT_MODEL_RATES.map((rate)=>[`${rate.model}:${rate.effectiveFrom}`,rate]));
    for(const rate of r.rows as ModelRate[]) rates.set(`${rate.model}:${rate.effectiveFrom}`,rate);
    return [...rates.values()];
  }
