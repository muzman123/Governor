...
export default async function WorkspaceLayout({children}:{children:React.ReactNode}) { try { const {session}=await requireWorkspaceSession(); const repositories=await accessibleRepositories(session); return <AppShell active="overview" login={session.githubLogin} repositories={repositories}>{children}</AppShell>; } catch { redirect("/api/auth/github/start"); } }
