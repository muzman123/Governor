...
export function PortfolioDashboard({items}:{items:RepositoryOverview[]}) {
  ...
  return <>
    ...
    <section className="panel workspace-receipts"><div className="panel-heading"><div><div className="eyebrow">Recent pull requests</div><h2>Receipts that changed the work</h2></div></div>{receipts.length?<div className="portfolio-list">{receipts.slice(0,8).map((item)=><Link className="portfolio-row" key={`${item.repo.id}-${item.prNumber}`} href={`/app/repos/${item.repo.slug}/pulls/${item.prNumber}`}><div><strong>{item.repo.slug} <span>#{item.prNumber}</span></strong><small>{item.title}</small><WorkContextInline context={item.workContext}/></div><div><strong>{money(item.totalCost)}</strong><small>{item.eventCount} usage events</small></div><span>-&gt;</span></Link>)}</div>:<div className="chart-empty">No pull-request receipts yet. Work with Codex on a branch, then push and open a pull request.</div>}</section>
  </>;
}
