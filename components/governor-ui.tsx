...
export function ReceiptDetail({receipt,repository}:{receipt:Receipt;repository:Repository}) {
  return <>
    ...
    <section className="receipt-hero"><div><span className="metric-label">Estimated Codex cost</span><strong>{money(receipt.totalCost)}</strong><small>{receipt.eventCount} usage events | {outcomeLabel(receipt.outcome)}</small></div><small className="receipt-hero-note">Transparent token-rate estimate</small></section>
