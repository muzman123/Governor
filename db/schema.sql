...
CREATE TABLE IF NOT EXISTS session_context_history (session_id TEXT NOT NULL, repository_slug TEXT NOT NULL, branch TEXT NOT NULL, head_sha TEXT NOT NULL, developer_id TEXT NOT NULL, observed_at TIMESTAMPTZ NOT NULL, PRIMARY KEY(session_id, observed_at));
CREATE INDEX IF NOT EXISTS session_context_history_lookup ON session_context_history(session_id, observed_at DESC);
...
CREATE TABLE IF NOT EXISTS repository_budgets (repository_id TEXT PRIMARY KEY REFERENCES repositories(id) ON DELETE CASCADE, monthly_budget_usd NUMERIC NOT NULL CHECK(monthly_budget_usd > 0), updated_by_developer_id TEXT NOT NULL REFERENCES developers(id) ON DELETE CASCADE, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS pull_requests (repository_id TEXT REFERENCES repositories(id), number INTEGER NOT NULL, branch TEXT NOT NULL, head_sha TEXT NOT NULL, title TEXT NOT NULL, state TEXT NOT NULL, outcome TEXT NOT NULL DEFAULT 'open', work_type TEXT NOT NULL DEFAULT 'unclassified', comment_id BIGINT, merged_at TIMESTAMPTZ, closed_at TIMESTAMPTZ, updated_at TIMESTAMPTZ NOT NULL, PRIMARY KEY(repository_id, number));
...
ALTER TABLE pull_requests ADD COLUMN IF NOT EXISTS work_type TEXT NOT NULL DEFAULT 'unclassified';
